...
import com.treevalue.robot.djl.transfer.stft.STFT
...
class Audio {
    ...
    private fun convertToTensor(
        ...
        val length = stftResult.size
        val width = stftResult[0].size
        ...
        for (i in 0 until length) {
            for (j in 0 until width) {
                magnitudes[i][j] = stftResult[i][j].abs().toFloat()
            }
        }
        return manager.create(magnitudes).reshape(Shape(1, length.toLong(), width.toLong()))
    }
